...
export const acmeDeployment = {
  ...
  environment: {
    ...
    optionalEnv: ["RAG_COMPANY_USE_CASE_ID", "RAG_COMPANY_PACK_CONTRACT_MODE"]
  },
  evals: {
    requiredPaths: ["profiles/acme/support/golden.jsonl", "profiles/acme/support/adversarial.jsonl"]
  },
  smoke: {
    validateCommand:
      "npm run company:validate -- --module dist/company/acme.company.js --export acmeDeployment --run-pack-contracts --use-case support",
    postgresSmokeCommand:
      "npm run company:smoke:postgres -- --module dist/company/acme.company.js --export acmeDeployment --use-case support"
  }
} satisfies CompanyDeploymentManifest;
```
...
Validate a company deployment manifest in CI after build:
...
The validator prints a safe summary with company identity, generated profile IDs, namespaces, source IDs, adapter IDs, deployment export path, required env names, eval paths, smoke commands, adapter-pack coverage, and readiness issues. It does not load adapters from env variables or print source bodies, connector credentials, API keys, principal claims, or retrieved context.

Add full pack contract execution to the same deployment check:
...
The validator accepts either the standard `CompanyDeploymentManifest` export shaped like `{ company, adapterPacks, environment, evals, smoke }`, or a legacy `CompanyProfile` export plus adapter-pack exports. It writes `company-deployment.json` and, when pack contracts run, `company-pack-contracts.json`. The pack report includes adapter, parser, source connector, permission mapper, coverage, issue-code, source-id, mode, count, and ledger summaries; it does not include connector records, parser bodies, warning payloads, source bodies, credentials, or principal claims.

Production startup loads the compiled company deployment manifest directly:
